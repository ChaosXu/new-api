...
Gin 请求处理链的通用中间件层（不含会话鉴权，鉴权归"鉴权"域）。提供跨域、限流、缓存、请求分发、人机验证、请求体控制、panic 恢复、server/internal/i18n 语言解析等请求处理能力，以及部分非会话的访问控制（管理员/Root/Token 校验）。
...
- **国际化能力**：从请求解析用户语言（GetLanguage），注入 server/internal/i18n 上下文。
