...
- **职责**：基于 GORM 的数据访问层，定义各实体表结构（用户/令牌/渠道/日志/兑换码/任务等），管理数据库连接与跨库（主库/日志库）兼容，提供查询与 CRUD；含缓存。
- **覆盖代码**：`model/`（全部实体与数据访问文件）
- **关键契约**：各实体 struct（User/Token/Channel/Log/Redemption 等）、`lockForUpdate(tx)` 行锁辅助、主库/日志库分支
