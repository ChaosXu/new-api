...
sequenceDiagram
    ...
    participant Http as HTTP 客户端<br/>(service SSRF 防护)
...
1. **请求到达 + 鉴权**（中间件）：`GET /videos/:task_id/content` 经 `TokenOrUserAuth`（会话或 Token 均可）与 `RouteTag("relay")` 路由标记后进入 `controller.VideoProxy`（`controller/video_proxy.go:33`）。
2. **任务结果取回 + 状态门控**（数据访问）：`model.GetByTaskId(userID, taskID)` 取任务记录（`controller/video_proxy.go:41`）；**状态门控**——仅 `task.Status == TaskStatusSuccess` 继续（line 52），非成功直接返回。随后 `model.CacheGetChannel(task.ChannelId)` 取渠道（line 58）。
...
4. **按渠道类型解析产物 URL**（渠道适配框架 / 控制器）：
   - **Gemini**：`getGeminiVideoURL` 经 `relay.GetTaskAdaptor` 调 `adaptor.FetchTask` + `ParseTaskResult` 取 `RemoteUrl`，附 `x-goog-api-key`（`controller/video_proxy_gemini.go:15`）。
   - **Vertex AI**：`getVertexVideoURL` 同走 TaskAdaptor，且当负载含 `bytesBase64Encoded` 时合成 data URL（`controller/video_proxy_gemini.go:148`）。
   - **OpenAI/Sora**：拼 `baseURL/v1/videos/{upstreamId}/content`，附 `Authorization: Bearer`（`controller/video_proxy.go:115`）。
   - **其他**：`task.GetResultURL()`（line 119）。
5. **data URL 直发分支**：若 videoURL 以 `data:` 开头，`writeVideoDataURL` base64 解码后直发原始字节（`controller/video_proxy.go:129,185`），不经上游。
