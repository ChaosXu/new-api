...
sequenceDiagram
    ...
    participant Codex as Codex 集成<br/>(service)
    ...
    loop 每个渠道
        ...
        alt 过期时间缺失 或 距过期 ≤24h
            ...
            Http->>OAuth: POST oauth/token<br/>(grant_type=refresh_token)
...
1. **调度启动 + 主节点门控**（后台调度）：`service.StartCodexCredentialAutoRefreshTask()`（`main.go:126` 调用）经 `codexCredentialRefreshOnce.Do` 幂等启动；**主节点门控**——`if !common.IsMasterNode { return }`（`service/codex_credential_refresh_task.go:37`），非主节点不运行。`gopool.Go` 立即执行一次后每 `codexCredentialRefreshTickInterval = 10 min` tick（line 44-50）。
