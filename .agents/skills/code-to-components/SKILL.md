...
从当前项目的所有代码中归纳出**组件**——即可独立开发、部署、运行的执行单元。产出分两层：

- **总览**（`components.md`）：组件清单表，每行一个组件，仅含组件名、路径、一句话职责，不含技术细节。
- **详情**（每个组件一份 `overview.md`）：承载该组件的技术栈 / 构建部署方式 / 制品形态 / 对外功能 / 边界与依赖。
...
4. **按模板产出**：读取两个模板 `assets/components.md`（总览）与 `assets/overview.md`（详情）。
   - 组件命名用仓库内的实际名称（目录名 / module 名 / 制品名），不臆造。
   - 先填总览：每个组件一行，仅含组件名、路径、一句话职责，不含技术细节；删除模板中的 HTML 注释后交付。
   - 再为每个组件填一份 overview 详情，字段照模板，删除 HTML 注释后交付。
5. **写入文件**：
   - 总览写入 `specs/03-ARCH/components.md`。
   - 每个组件的详情写入 `specs/03-ARCH/{组件名}/overview.md`（目录不存在则创建）。总览与详情通过同名 `{组件名}` 子目录关联，不在总览表中放置详情链接。
