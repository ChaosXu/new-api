...
```
server/                    — Backend Go module root (go.mod lives here)
  cmd/new-api/             — Application entry point (package main)
  internal/                — Private application code (Go internal/ visibility)
    router/                — HTTP routing (API, relay, dashboard)
  ...
  relaykit/                — Independently buildable relay conversion Go module
  pkg/                     — Internal packages (cachex, ionet, billingexpr, perf_metrics)
web/                       — Frontend (React 19, Rsbuild, Base UI, Tailwind) — deployed separately
...
- Any change affecting `relaykit/` or its public APIs MUST be verified with `cd server/relaykit && GOWORK=off go build ./...`; a successful root-module build is not sufficient.

**JSON package:** All JSON marshal/unmarshal operations MUST use the wrapper functions in `server/internal/common/json.go`:
...
- When raw SQL is unavoidable, account for dialect differences:
  ...
  - Use `commonGroupCol`, `commonKeyCol` from `server/internal/model/main.go` for reserved-word columns like `group` and `key`.
...
- Migrations must work on all three databases. For SQLite, use `ALTER TABLE ... ADD COLUMN` instead of `ALTER COLUMN` (see `server/internal/model/main.go` for patterns).
...
- Regression tests for these invariants belong with the boundary they protect (request validators, converter helpers). See `server/internal/relay/helper/openai_image_request_test.go`, `server/internal/relay/common/relay_utils_test.go`, and `server/internal/common/quota_math_test.go` for the expected style.
